...
important_columns = ["track_name", "strand", "variant_id", "log10_median_of_exon_medians_alt", "log10_median_of_exon_medians_ref"]
...
for i, gene in enumerate(tqdm(gene_list, desc="Processing genes"), start=1):
    ...
    df_wide = df.pivot_table(
        index="variant_id",
        columns="track_name",
        values=["log10_median_of_exon_medians_alt", "log10_median_of_exon_medians_ref"],
        aggfunc="mean"
    )
    df_wide.columns = [f"{track}_{'alt' if 'alt' in val else 'ref'}" for val, track in df_wide.columns]
...
# output to file
output_path = "results_logistic/VS_per_track_strand_with-.parquet"
