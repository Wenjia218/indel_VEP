...
parser.add_argument("--batch_idx", type=int, required=True, help="0-based batch index")
...
# Slice this batch's genes
batch_genes = [g for i, g in enumerate(gene_list) if i % args.n_batches == args.batch_idx]
print(f"Batch {args.batch_idx}/{args.n_batches}: processing {len(batch_genes)} genes")

base_path = "/s/project/benchmark-lm/data/orthology_primates/preds"
important_columns = ["track_name", "strand", "variant_id",
                     "log10_median_of_exon_medians_alt"]
...
output_path = f"results_logistic/batch_features/batch_{args.batch_idx:04d}_of_{args.n_batches:04d}.parquet"

all_gene_features = []

for gene in tqdm(batch_genes, desc=f"Batch {args.batch_idx}"):
    f = os.path.join(
        base_path,
...
if all_gene_features:
    batch_result = pd.concat(all_gene_features, ignore_index=True)
    del all_gene_features
    gc.collect()
    print(f"Batch {args.batch_idx} shape: {batch_result.shape}")
    print(f"NaN rate: {batch_result.isna().mean().mean():.1%}")
    batch_result.to_parquet(output_path, index=False)
    print(f"Saved to {output_path}")
else:
    print(f"Batch {args.batch_idx}: no data written (all genes missing)")
